...
function pintarTotal(suma) {
  ...
  pSuma.textContent = `TOTAL: ${suma} €/h`;
...
function createSel() {
  ...
  mainElement.append(fElement);
  fElement.append(selElement);
  selElement.append(opElement);
...
function getHora(){
  ...
  selectElement.addEventListener("change", (evento) => {
    ...
    if (op >= 0 && op <= 24) {
      const arrPrices = [...JSON.parse(window.localStorage.getItem(
        "Precios"
      ))];
